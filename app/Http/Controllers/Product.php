...
class Product extends Controller
{
    ...
    public function add_product (){
        $this->AuthenLogin();
        $cate_product = DB::table('pdh_category_product')->orderby('category_id','desc')->get();
        $brand_product = DB::table('pdh_brand_product')->orderby('brand_id','desc')->get();
    ...
    public function all_product (){
        ...
        $all_product = DB::table('pdh_product')
        ->join('pdh_category_product','pdh_category_product.category_id','=','pdh_product.category_id')
        ->join('pdh_brand_product','pdh_brand_product.brand_id','=','pdh_product.brand_id')
        ->orderby('pdh_product.product_id','desc')->paginate(5);
    ...
    public function save_product (AddProductRequests $request){
        ...
        if($get_image){
            ...
            DB::table('pdh_product')->insert($data);
    ...
    public function unactive_product ($product_id){
        $this->AuthenLogin();
        DB::table('pdh_product')->where('product_id',$product_id)->update(['product_status'=>1]);
        Session::put('message','Hiện Sản Phẩm Thành Công');
        return Redirect::to('all-product');
    }
    public function active_product ($product_id){
        $this->AuthenLogin();
        DB::table('pdh_product')->where('product_id',$product_id)->update(['product_status'=>0]);
    ...
    public function edit_product($product_id){
        ...
        $edit_product = DB::table('pdh_product')->where('product_id',$product_id)->get();
    ...
    public function update_product(Request $request ,$product_id){
        ...
        if($get_image){
            ...
            $get_image->move('upload/product',$new_image);
            $data['product_image'] = $new_image; 
            DB::table('pdh_product')->where('product_id',$product_id)->update($data);
    ...
    public function chi_tiet($product_slug , Request $request){
        $cate_product  = DB::table('pdh_category_product')->where('category_status','1')->orderby('category_id','desc')->get();
        $brand_product = DB::table('pdh_brand_product')->where('brand_status','1')->orderby('brand_id','desc')->get();
        $chitiet_product = DB::table('pdh_product')
        ->join('pdh_category_product','pdh_category_product.category_id','=','pdh_product.category_id')
        ->join('pdh_brand_product','pdh_brand_product.brand_id','=','pdh_product.brand_id')->where('pdh_product.product_slug',$product_slug)->get();
        ...
        $related_product = DB::table('pdh_product')
        ->join('pdh_category_product','pdh_category_product.category_id','=','pdh_product.category_id')
        ->join('pdh_brand_product','pdh_brand_product.brand_id','=','pdh_product.brand_id')
        ->where('pdh_category_product.category_id',$category_id)->whereNotIn('pdh_product.product_slug',[$product_slug])->get();
